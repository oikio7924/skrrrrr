<template>
  ...
  <main class="ob-screen" :class="{ 'is-last': isLast }">
    <header class="top">
      <button v-if="isLast" class="back" @click="prev">뒤로가기</button>
      <button v-else="!isLast" class="skip" @click="goLast">건너뛰기</button>
    </header>
    ...
    <div
      ...
      @pointerup.passive="onUp"
      @pointercancel.passive="onUp"
    ...
    >
      ...
      <!-- 2. AI 학습(목소리/얼굴 → AI 캐릭터/목소리) -->
      <section class="slide">
        <div class="copy">
          <h2>나만의 AI 목소리/캐릭터 !</h2>
          <p>목소리·얼굴을 학습해 나만의 AI로.</p>
        </div>
        <img :src="imgs.ai" alt="AI 학습" class="hero" />
      </section>
...
function goLast() { step.value = COUNT - 1 }
...
.ob-screen.is-last { grid-template-rows: auto 1fr; }
...
.slide {
  width: 100%; height: 100%;
  display: grid;
  grid-template-rows: 1fr auto auto 1fr; /* 위 여백 / 텍스트 / 이미지 / 아래 여백 */
  justify-items: center; align-items: center;
  row-gap: 12px; padding: 0 16px;
}
.copy { grid-row: 2; text-align: center; margin: 0; }
h2 { font-size: 28px; line-height: 1.25; margin: 0 0 8px; font-weight: 900; color: #111; text-align: center; }
p  { font-size: 15px; color: #555; margin: 0; text-align: center; }

/* 모든 히어로 이미지 공통(최대 300px) */
.hero { grid-row: 3; width: min(300px, 72%); max-width: 320px; display: block; }

/* ───────── 마지막 슬라이드 ───────── */
.slide--final { position: relative; padding: 0 16px; }

/* ───────── 마지막 슬라이드 크게 보이도록 업스케일 ───────── */
...
.slide--final .logo {
  ...
  width: min(360px, 80vw);   /* ✅ 여기만 키우면 됩니다 */
  max-width: 200px;
  height: auto;
}
...
.slide--final .final-copy {
  ...
  left: 50%;
  transform: translateX(-50%);
  top: calc(50% + 40px);     /* 기존 +30px → +40px 로 살짝 아래에서 시작 */
  margin: 0;
  line-height: 1.55;
  font-weight: 800;
  font-size: clamp(20px, 3.4vw, 26px);  /* ✅ 크기 업 */
}

/* 다크 모드 색상은 기존 그대로 두셔도 됩니다 */
...
.slide--final .cta {
  ...
  background: #B487FF; color: #fff; border: 0;
  font-weight: 900; font-size: 18px;
}

/* dots / controls */
.dots { display: flex; justify-content: center; gap: 8px; padding: 8px 0; }
.dot { width: 8px; height: 8px; border-radius: 999px; border: 0; background: #d8cdf9; }
.dot.active { width: 22px; background: #7a54cf; transition: width .2s; }

.controls { display: flex; justify-content: center; gap: 12px; padding-bottom: 8px; }
.primary, .ghost { min-width: 120px; padding: 10px 16px; border-radius: 12px; font-weight: 800; cursor: pointer; }
.primary { background: #A993E8; color: #fff; border: 0; }
.ghost { background: transparent; border: 1px solid #d4c8ff; color: #7a54cf; }


/* 다크 */
@media (prefers-color-scheme: dark) {
  .ob-screen { background: #0f0f14; }
  h2 { color: #f5f1ff; }
  p  { color: #cbbff6; }
  .final-copy { color: #e9e2ff; }
  .ghost { border-color: #7158dd; color: #cbbff6; }
}
</style>
