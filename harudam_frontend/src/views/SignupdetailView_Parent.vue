<template>
  <div class="signup-overlay">
    <div class="signup-card" role="dialog" aria-labelledby="signupTitle">
      <button class="close-btn" aria-label="닫기" @click="$router.back()">×</button>
      ...
      <form class="form" @submit.prevent="onSubmit">
        <label class="field">
            ...
            <p v-if="touched.name && !valid.name" class="hint invalid">2글자 이상 입력해주세요.</p>
        </label>
        <div class="field">
            <label class="label">부모님 휴대폰 번호</label>
            <div class="row">
                <input v-model="form.phone" type="tel" inputmode="numeric" maxlength="13" placeholder="예) 010-1234-5678" required @input="maskPhone" @blur="touched.phone = true" />
                <button type="button" class="ghost-btn" :disabled="!valid.phone || sending || countdown > 0" @click="openConsentModal">
                    <template v-if="countdown > 0">{{ countdown }}초</template>
                    <template v-else>인증번호 전송</template>
                </button>
            </div>
            <p v-if="touched.phone && !valid.phone" class="hint invalid">휴대폰 번호 형식을 확인해주세요.</p>
        </div>
        ...
        <div class="field">
          ...
          <div class="row">
            ...
            <button
              type="button"
              class="ghost-btn"
              :disabled="!valid.code || verifyingCode || codeVerified"
              @click="verifyCode"
            >
          ...
          <p v-if="touched.code && !valid.code" class="hint invalid">6자리 숫자를 입력해주세요.</p>
        ...
        <label class="field">
            ...
            <p v-if="touched.birth && !valid.birth" class="hint invalid">생년월일을 선택해주세요.</p>
        </label>
        ...
        <img v-if="photoPreviewUrl" :src="photoPreviewUrl" alt="부모님 사진 미리보기" class="photo-preview" />
      </div>
        <!-- 주소 -->
          <fieldset class="field">
            <legend class="label">주소(선택)</legend>
            <input
              v-model.trim="form.addr1"
              type="text"
              placeholder="주소 검색"
              readonly
        ...
        <fieldset class="field">
            ...
            <div class="radio-row" role="radiogroup" aria-label="성별">
                <label class="radio"><input type="radio" value="F" v-model="form.gender" /><span>여성</span></label>
                <label class="radio"><input type="radio" value="M" v-model="form.gender" /><span>남성</span></label>
            </div>
            <p v-if="touched.gender && !valid.gender" class="hint invalid">성별을 선택해주세요.</p>
        </fieldset>
        <button type="submit" class="primary-btn submit-btn" :disabled="!formValid || submitting">하루담 시작하기</button>
      </form>
    </div>
    ...
    <div v-if="showConsentModal" class="modal-overlay" @click.self="closeConsentModal">
      <div class="modal-card">
        ...
        <div class="modal-content">
          ...
          <p class="details">
            자세한 내용은 <a href="#" @click.prevent="onDetailsClick">개인정보처리방침</a>에서 확인하실 수 있습니다.
          </p>
        </div>
...
.signup-card {
  /* width: 100px;  <-- 이 줄 제거! */
  width: clamp(320px, 92vw, 420px); /* 최소 320px, 화면의 92%, 최대 420px */
  background: #ffffff;
  border-radius: 16px;
...
.row {
  display: grid;
  grid-template-columns: 1fr auto; /* 입력칸이 넓게 */
  gap: 8px;
  align-items: center;
}
