        ...
        <div class="content-wrapper">
          ...
          <section class="card" aria-labelledby="sec1">
            ...
            <div class="row">
              ...
              <img v-if="photoPreviewUrl" :src="photoPreviewUrl" alt="자녀 사진 미리보기" class="photo-preview" />
            </div>
            ...
            <div class="row">
              ...
              <div class="field voice-field">
                ...
                <button v-if="!isRecording && !recordedAudioUrl" @click="startRecording" type="button" class="micro-btn">녹음 시작</button>
                <button v-if="isRecording" @click="stopRecording" type="button" class="micro-btn stop">녹음 중지</button>
                <button v-if="!isRecording && recordedAudioUrl" @click="resetRecording" type="button" class="micro-btn">다시 녹음</button>
              </div>
            ...
            <div class="row">
              ...
              <div class="field">
                <input v-model.trim="form.address" type="text" placeholder="주소 검색" readonly @click="handleAddressSearch"/>
          ...
          <!-- [수정] type을 submit으로 변경하고 @click을 제거 -->
          <button
            type="submit"
            class="cta"
...
:root { --brand:#A993E8; --text:#222; --muted:#777; --line:#eaeaf0; }
...
.topbar {
  ...
  height: 52px; display: grid; grid-template-columns: 52px 1fr 52px;
  align-items: center; padding: 0 4px; background: #fff; border-bottom: 1px solid var(--line);
  flex-shrink: 0;
}
...
.icon-btn.ghost { color:#52525b; } .spacer { width: 48px; }
...
fieldset.row { border: none; padding: 0; }
.label { display: block; font-size: 13px; font-weight: 700; margin-bottom: 6px; color: var(--text); }

.field {
  display: flex; align-items: center; gap: 8px;
  background: #f8f8fb; border: 1px solid #ececf2; border-radius: 12px; padding: 0 12px; height: 48px;
}
.field input { flex: 1; border: none; background: transparent; outline: none; font-size: 15px; width: 100%; }
.field input[readonly] { color: var(--muted); cursor: pointer; }
.field input[type="date"] { position: relative; }
.field input[type="date"]::-webkit-calendar-picker-indicator {
  position: absolute; right: 0; top: 0; width: 100%; height: 100%; padding: 0; color: transparent; background: transparent; cursor: pointer;
}

.micro-btn {
  border: 1px solid #c9b7ff; background: #fff; border-radius: 14px;
  padding: 7px 12px; font-size: 12px; color: var(--brand); cursor: pointer; flex-shrink: 0;
}
...
.agreements-title { font-size: 15px; font-weight: 700; margin-bottom: 10px; }
.agreements-body { padding-top: 5px; }
.divider { border: none; border-top: 1px solid var(--line); margin: 10px 0; }
.check { position: relative; display: flex; align-items: center; gap: 10px; margin: 12px 0; font-size: 14px; cursor: pointer; }
